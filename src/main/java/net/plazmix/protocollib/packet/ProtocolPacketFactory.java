...
public class ProtocolPacketFactory {
    ...
    public WrapperPlayServerNamedEntitySpawn createNamedEntitySpawnPacket(int entityId, WrappedDataWatcher dataWatcher, UUID uuid, Location location) {
        ...
        packet.getHandle().getSpecificModifier(UUID.class).write(0, uuid);

        packet.getHandle().getIntegers().write(1, (int) Math.floor(location.getX() * 32.0D));
        packet.getHandle().getIntegers().write(2, (int) Math.floor(location.getY() * 32.0D));
        packet.getHandle().getIntegers().write(3, (int) Math.floor(location.getZ() * 32.0D));

        packet.getHandle().getBytes().write(0, (byte) (location.getYaw() * 256.0F / 360.0F));
        packet.getHandle().getBytes().write(1, (byte) (location.getPitch() * 256.0F / 360.0F));

        packet.getHandle().getDataWatcherModifier().write(0, dataWatcher);
    ...
    public WrapperPlayServerSpawnEntity createSpawnEntityPacket(int entityId, int spawnTypeId, int entityData, Location location) {
        ...
        packet.getHandle().getIntegers().write(0, entityId);
        packet.getHandle().getIntegers().write(9, spawnTypeId);
        packet.getHandle().getIntegers().write(10, entityData);
     ...
     // if (entityData > 0) {
     //     packetContainer.getIntegers().write(4, (int) (velocity.getX() * 8000.0D));
     //     packetContainer.getIntegers().write(5, (int) (velocity.getY() * 8000.0D));
     //     packetContainer.getIntegers().write(6, (int) (velocity.getZ() * 8000.0D));
     // }

        packet.getHandle().getIntegers().write(7, (int) (location.getYaw() * 256.0F / 360.0F));
        packet.getHandle().getIntegers().write(8, (int) (location.getPitch() * 256.0F / 360.0F));
    ...
    public WrapperPlayServerSpawnEntityLiving createSpawnEntityLivingPacket(int entityId, EntityType entityType, WrappedDataWatcher dataWatcher, Location location) {
        ...
        packet.getHandle().getIntegers().write(0, entityId);
        packet.getHandle().getIntegers().write(1, (int) entityType.getTypeId());

        packet.getHandle().getIntegers().write(2, (int) Math.floor(location.getX() * 32.0D));
        packet.getHandle().getIntegers().write(3, (int) Math.floor(location.getY() * 32.0D));
        packet.getHandle().getIntegers().write(4, (int) Math.floor(location.getZ() * 32.0D));
