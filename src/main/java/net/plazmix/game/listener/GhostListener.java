...
public final class GhostListener extends GameListener {
    ...
    public void onDamageSelf(EntityDamageEvent event) {
        ...
        if (event.getEntity() instanceof Player && GameUser.from((Player) event.getEntity()).isGhost()) {
            event.setCancelled(true);
        }
    }
    ...
    public void onGhost(GameGhostChangeEvent event) {
        ...
        if (event.isGhost()) {
            ...
            // Reset loaded protocol teams.
            for (ProtocolTeam loaded : ProtocolTeam.findReceiverList(player)) {
                loaded.addReceiver(player);
            }
        }
    }
