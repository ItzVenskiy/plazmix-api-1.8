...
public final class PlayerGamesData {
    ...
    public void insert(@NonNull MysqlDatabaseConnection connection) {
        connection.execute(true, "INSERT INTO `PlayerGamesData` (`Game`, `StartDate`, `EndDate`, `Map`, `Server`, `Players`, `Winner`) VALUES (?,?,?,?,?,?,?)",
                game,
                new Timestamp(startTimeMillis),
                new Timestamp(endTimeMillis),
                map,
                server,
                playersIdsList.toString().replace("[", "").replace("]", ""),
                winner.replace(" ", "").replace(",", ", ").replace("&", ", "));
    }
