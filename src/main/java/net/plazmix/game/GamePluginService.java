...
public final class GamePluginService {
    ...
    public void resetData() {
        this.resetData(GamePlugin.getInstance());
    }
    public void resetData(GamePlugin gamePlugin) {
        teamManager.getLoadedTeamMap().clear();
        teamManager.getInternalColorIdMap().clear();

        stateManager.getGameStates().clear();

        gameSettingsMap.clear();
        gameDatabasesMap.clear();
        gameItemCategoriesMap.clear();
        gameUsers.clear();

        BaseScoreboardScope.SCOPING_SCOREBOARD_MAP.values().forEach(baseScoreboard -> baseScoreboard.cancelUpdaters(true));

        for (BukkitTask gameScheduler : GameSchedulers.getPendingTasks()) {
            Bukkit.getScheduler().cancelTask(gameScheduler.getTaskId());
        }

        Bukkit.getScheduler().cancelTasks(gamePlugin);

        HandlerList.unregisterAll(gamePlugin);
    }
